...
function PlaceCardItem({ place }) {
  ...
  useEffect(() => {
    // Fetch place details and photo when component mounts or place changes
    const fetchPlaceId = async () => {
      ...
      try {
        ...
        // Make API request to get place details
        ...
        if (response.data.results && response.data.results.length > 0) {
          // Select a photo from the middle of the results array
  ...
  return (
    <Link to={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(place?.placeName)}`} target="_blank" rel="noopener noreferrer">
      <div className="border rounded-xl p-3 mt-2 flex gap-5 hover:scale-105 transition-all hover:shadow-md cursor-pointer">
        {/* Conditional rendering for image loading state */}
        {isLoading ? (
